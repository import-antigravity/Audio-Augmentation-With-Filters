...
def train(data, classifier, optimizer, epochs, batch_size, path, num_gpus):
    os.makedirs(path)
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(os.path.join(path, '{epoch:02d}-{val_loss:.2f}.hdf5'),
                                                    monitor='val_loss', verbose=0, save_best_only=True,
                                                    save_weights_only=False, mode='auto', period=1)
    csv_logger = tf.keras.callbacks.CSVLogger(os.path.join(path, "model_history_log.csv"), append=True)
    # Train model
