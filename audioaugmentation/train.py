...
def train(classifier, optimizer, epochs, batch_size, path):
    ...
    print('Training...')
    checkpoint = tf.keras.callbacks.ModelCheckpoint(path + os.linesep + '{epoch:02d}-{val_loss:.2f}.hdf5',
                                                    monitor='val_loss', verbose=0, save_best_only=True,
                                                    save_weights_only=False, mode='auto', period=1)
    csv_logger = tf.keras.callbacks.CSVLogger(path + os.linesep + "model_history_log.csv", append=True)
